...
public class CostTripEstimator extends AbstractTripRouterEstimator {
	@Inject
	public CostTripEstimator(TripRouter tripRouter, ActivityFacilities facilities,
	...
	private double calculateUtilityForWorkActivity(String mode, double totalTravelTime, double totalTravelDistance, boolean carAlwaysAvailable) {
		...
		if (carAlwaysAvailable) {
			...
			switch (mode) {
				...
				case TransportMode.drt: utility = -0.7 - 2.94 * totalTravelTime - 0.0013 * (totalTravelDistance * 40) ;break;
			}
		} else {
			switch (mode) {
		...
		if (carAlwaysAvailable) {
			switch (mode) {
				...
				case TransportMode.drt:
					utility = -0.7 -2.94 * totalTravelTime - 0.0013 * (totalTravelDistance * 40) ;
					break;
			}
		} else {
			switch (mode) {
