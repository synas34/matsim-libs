...
public class Jan13TripEstimator extends AbstractTripRouterEstimator {
	@Inject
	public Jan13TripEstimator(TripRouter tripRouter, ActivityFacilities facilities,
	...
	private static Collection<String> createPreroutedModes() {
		...
		return Arrays.asList(TransportMode.car, TransportMode.walk, TransportMode.pt, TransportMode.bike, TransportMode.ride, TransportMode.drt);
	}
	@Override
		...
		for (PlanElement element : routedTrip) {
			if (element instanceof Leg) {
				...
				if (leg.getMode().equals("transit_walk")) {
					totalRidingTime = totalTransferDistance ;
					totalTransferTime += leg.getRoute().getDistance() * 1e-3;
				} else {
					totalTransferTime += (leg.getTravelTime().seconds() / 3600.0);
					totalTransferDistance += leg.getRoute().getDistance() * 1e-3;
