...
public class SAVasRideTripEstimator extends AbstractTripRouterEstimator {
	@Inject
	public SAVasRideTripEstimator(TripRouter tripRouter, ActivityFacilities facilities,
	...
	private double calculateUtilityForWorkActivity(String mode, double totalTravelTime, double totalTravelDistance, boolean carAlwaysAvailable) {
		...
		if (totalTravelDistance <= 4){
			switch (mode) {
				...
				case TransportMode.drt: utility = -3.35 - 7.65 * totalTravelTime - 0.0013 * (totalTravelDistance * 70) ;break;			}
	...
	private double calculateStandardUtility(String mode, double totalTravelTime, double totalTravelDistance,  boolean carAlwaysAvailable) {
		...
		if (totalTravelDistance <= 4){
			switch (mode) {
				case TransportMode.car: utility = -0.96 - 5.3 * totalTravelTime - 0.0013 * (totalTravelDistance * 10); break;
