...
public class UrbanIndexTripEstimatorPASS extends AbstractTripRouterEstimator {
	@Inject
	public UrbanIndexTripEstimatorPASS(TripRouter tripRouter, ActivityFacilities facilities,
...
//		}
	...
	private double calcModeUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, double totalRidingDistance, String Student,String Teikiken, double PC1, double PC2, DiscreteModeChoiceTrip trip) {
		...
		if (totalTravelDistance <= 3) {
			...
			bikeFactor = -1.79 - 6.00 * totalTravelTime ;
		...
		switch (mode) {
			...
			case TransportMode.car: utility = carFactor -1.28 - 4.96 * totalTravelTime - 0.0014 * (totalTravelDistance * 8) - 0.37 * PC1 ; break;
			case TransportMode.ride: utility = rideFactor - 2.64 - 6.59	 * totalTravelTime - 0.16 * PC1 ;		break;
			case TransportMode.pt: utility = -2.78 - 0.295 * totalRidingTime - 3.07 * totalTransferTime - 0.0014 * (totalRidingDistance * 16) + 0.6 * teikiken_dummy + 0.035 * PC1  ;break;
			case TransportMode.drt, TransportMode.drtA, TransportMode.drtC, TransportMode.drtD, TransportMode.drtB, TransportMode.drtE:
				utility = -2.64  - (6.59 * totalTravelTime) - 0.0014 * ((totalTravelDistance * 70) + 150)  ; break; }
