...
public class Jan12TripEstimator extends AbstractTripRouterEstimator {
	@Inject
	public Jan12TripEstimator(TripRouter tripRouter, ActivityFacilities facilities,
...
//		utility = calcModeUtil(mode, totalTravelTime, totalTravelDistance, totalRidingTime, totalTransferTime, carAlwaysAvailable);
	...
	private double calcModeUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, boolean carAlwaysAvailable) {
		...
		switch (mode) {
			...
			case TransportMode.car: utility = -2.41 - 5.50 * totalTravelTime - 0.0016 * (totalTravelDistance * 7); break;
			case TransportMode.ride: utility = -3.58 - 6.95	 * totalTravelTime;		break;
			case TransportMode.pt: utility = -2.05 - 0.32 * totalRidingTime - 4.30 * totalTransferTime - 0.0015 * (totalTravelDistance * 16) ;break;     }
	...
	private double UrbanWorkUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, boolean carAlwaysAvailable) {
		...
		if (totalTravelDistance <= 3) {
			walkFactor =  -6.76 * totalTravelTime;
			bikeFactor = -1.14 - 6.03 * totalTravelTime ;
		} else if (totalTravelDistance > 3 && totalTravelDistance <= 7) {
			walkFactor = dummyvalue + dummyvalue * totalTravelTime;
			bikeFactor = -1.14 - 6.03 * totalTravelTime;
		} else {
		...
		switch (mode) {
			case TransportMode.walk: utility = walkFactor    ;break;
			case TransportMode.bike: utility = bikeFactor 	;break;
			case TransportMode.car: utility = -3.13 - 3.52 * totalTravelTime - 0.0052 * (totalTravelDistance * 7); break;
			case TransportMode.ride: utility = 1.25 * -4.14 - 6.69	 * totalTravelTime;break;
			case TransportMode.pt: utility = -0.89 - 0.23 * totalRidingTime - 6.76 * totalTransferTime - 0.0010 * (totalTravelDistance * 16) ;break;     }
	...
	private double UrbanOtherUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, boolean carAlwaysAvailable) {
		...
		switch (mode) {
			...
			case TransportMode.ride: utility = 1.25 * -4.14 - 6.64 * totalTravelTime;break;
	...
	private double SuburbanWorkUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, boolean carAlwaysAvailable) {
		...
		if (totalTravelDistance <= 3) {
			walkFactor = -5.41 * totalTravelTime;
			bikeFactor = -1.14 - 4.90 * totalTravelTime;
		} else if (totalTravelDistance > 3 && totalTravelDistance <= 7) {
			walkFactor = dummyvalue + dummyvalue * totalTravelTime;
			bikeFactor = -1.14 - 4.90 * totalTravelTime;
		} else {
		...
		switch (mode) {
			case TransportMode.walk: utility = walkFactor     ;break;
			case TransportMode.bike: utility = bikeFactor 	  ;break;
			case TransportMode.car: utility = -0.87	 - 5.01 * totalTravelTime - 0.0030 * (totalTravelDistance * 7); break;
			case TransportMode.ride: utility = 1.25 * -2.58 - 6.68 * totalTravelTime;break;
			case TransportMode.pt: utility = -0.75 - 0.86 * totalRidingTime - 5.41 * totalTransferTime - 0.0005 * (totalTravelDistance * 16) ;break;     }
	...
	private double SuburbanOtherUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, boolean carAlwaysAvailable) {
		...
		switch (mode) {
			...
			case TransportMode.ride: utility = 1.25 * -2.79 - 6.20 * totalTravelTime ;break;
	...
	private double CBDWorkUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, boolean carAlwaysAvailable) {
		...
		if (totalTravelDistance <= 3) {
			walkFactor = -7.34 * totalTravelTime;
			bikeFactor = -2.53 - 6.52 * totalTravelTime;
		} else if (totalTravelDistance > 3 && totalTravelDistance <= 7) {
			walkFactor = dummyvalue * totalTravelTime;
			bikeFactor = -2.53 - 6.52 * totalTravelTime;
		} else {
		...
		switch (mode) {
			case TransportMode.walk: utility = walkFactor    ;break;
			case TransportMode.bike: utility = bikeFactor 	 ;break;
			case TransportMode.car: utility = -5.37 - 2.78 * totalTravelTime - 0.0025 * (totalTravelDistance * 7); break;
			case TransportMode.ride: utility = 1.25 * -5.91 - 4.38 * totalTravelTime;break;
			case TransportMode.pt: utility = -1.46 + 0.276 * totalRidingTime - 7.34 * totalTransferTime - 0.0008 * (totalTravelDistance * 16) ;break;     }
		return utility;
	}
	...
	private double CBDOtherUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, boolean carAlwaysAvailable) {
		...
		switch (mode) {
			...
			case TransportMode.ride: utility = 1.25 * -3.35 - 7.65 * totalTravelTime ;break;
	...
	private double RuralUtil(String mode, double totalTravelTime, double totalTravelDistance, double totalRidingTime, double totalTransferTime, boolean carAlwaysAvailable) {
		...
		if (totalTravelDistance <= 3) {
			walkFactor = -6.29 * totalTravelTime;
			bikeFactor = -1.12 - 5.63 * totalTravelTime;
		} else if (totalTravelDistance > 3 && totalTravelDistance <= 7) {
			walkFactor = dummyvalue * totalTravelTime;
			bikeFactor = -1.12 - 5.63 * totalTravelTime;
		} else {
		...
		switch (mode) {
			case TransportMode.walk: utility = walkFactor    ;break;
			case TransportMode.bike: utility = bikeFactor 	 ;break;
			case TransportMode.car: utility = -1.59 - 5.39 * totalTravelTime - 0.0033 * (totalTravelDistance * 7); break;
			case TransportMode.ride: utility = 1.25 * -3.06 - 7.70 * totalTravelTime  ;break;
			case TransportMode.pt: utility = -0.63 - 0.24 * totalRidingTime - 6.29 * totalTransferTime - 0.0006 * (totalTravelDistance * 16) ;break;     }
		return utility;
	}
}
