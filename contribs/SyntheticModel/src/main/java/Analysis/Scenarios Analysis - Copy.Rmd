...
trips3075 <- read_delim("trips(Req%30%75).csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
trips3050 <- read_delim("trips(Req%30%50).csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
trips5030 <- read_delim("trips(Req%50%30).csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
trips1250 <- read_delim("trips(Req%12%50).csv", 
    delim = ";", escape_double = FALSE, trim_ws = TRUE)
...
df1250 <- inner_join(df, trips1250, by = "person")
df3075 <- inner_join(df, trips3075, by = "person")
df3050 <- inner_join(df, trips3050, by = "person")
df5030 <- inner_join(df, trips5030, by = "person")
...
combined_data <- bind_rows(
  # mutate(summ_base, Category = "pop/200"),
  # mutate(summ1250, Category = "12%50(600)"),
  mutate(summ3075, Category = "30%75(360)"),
  mutate(summ3050, Category = "30%50(210)"),
  mutate(summ5030, Category = "50%30(95)"))

combined_data <- combined_data %>%
  filter(!(longest_distance_mode == "transit_walk")) 
...
combined_data$Category <- factor(combined_data$Category,
                                             levels = c("pop/200", "12%50(600)", "30%75(360)", "30%50(210)", "50%30(95)"))
...
ggplot(PT_df1 , aes(x = Category, y = Percentage, color = longest_distance_mode, group = longest_distance_mode)) +
  ...
  labs(title = "SAV Share Percentage by Fleet Size", x = "Fleet Size", y = "Percentage") +
  theme_minimal() +
  theme(
    legend.position = "bottom",
    text = element_text(size = 12),  # Increase general text size
  ...
  scale_color_brewer(palette = "Set2")  # Use a color palette that contrasts well with the background
